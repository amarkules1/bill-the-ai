...
class QuestionLikesRepository:
    def get_likes_for_question(self, question_id):
        conn = get_connection()
        query = sa.text(f"SELECT * FROM question_likes where question_id = :question_id")
    ...
    def get_likes_for_user(self, user_id):
        conn = get_connection()
        query = sa.text(f"SELECT * FROM question_likes where user_id = :user_id")
    ...
    def get_like_ct_for_question(self, question_id):
        conn = get_connection()
        query = sa.text(f"SELECT count(*) FROM question_likes where question_id = :question_id")
    ...
    def get_like_counts_for_questions_in_bill(self, bill_id):
        conn = get_connection()
        query = sa.text(f"SELECT question_id, count(*) FROM question_likes where bill_id = :bill_id group by question_id")
